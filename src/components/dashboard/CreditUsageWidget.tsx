...
export function CreditUsageWidget() {
  const [usage, setUsage] = useState<CreditUsageData | null>(null)
      ...
      <CardContent className="space-y-6">
        ...
        <div className="p-4 bg-primary/5 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">현재 보유 크레딧</span>
            <span className="text-2xl font-bold">{usage.currentCredits.toLocaleString()}</span>
          </div>
          {usage.savedThisMonth > 0 && (
            <div className="flex items-center gap-2 text-sm text-green-600">
              <TrendingDown className="h-4 w-4" />
              <span>이번 달 {usage.savedThisMonth.toLocaleString()} 크레딧 절약</span>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger>
        ...
        <div className="p-3 bg-blue-50 rounded-lg">
          <p className="text-xs text-blue-900">
            ...
            AI가 파일의 일부만 수정할 수 있는 경우, 사용된 크레딧의 절반만 차감됩니다.
          </p>
        </div>
      </CardContent>
    </Card>
